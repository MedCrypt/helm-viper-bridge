...
parser = argparse.ArgumentParser(
    prog='Helm - List Unmatched SBOM entries',
...
parser.add_argument('-pn', '--product_name', help="The product name to list the unmatched sbom entries", required=True)
parser.add_argument('-v', '--version', help="The version to list the unmatched sbom entries", required=True)
...
if response.status_code  == 200:
    ...
    if response.status_code == 200:
        ...
        if response.status_code == 200:
            ...
            url = base_url + "/listunmatchedsbomentries"
            payload = heim_sbom.ListSbomEntries()
            payload.request.organization_product_version_id.uuid = product_version.id.uuid
            response = requests.post(url, data=payload.SerializeToString(), headers=auth_headers, verify=args.verify)
            msg = heim_sbom.ListSbomEntries.FromString(response.content)
